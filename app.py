"""
Tarot Predictor V11.1
Methodology: a team CONTROLS the Match Force via mirror (SIM) or destabiliser (OPP).
Rules engine: v3 (analysis_engine.py).

What's new in V11.1:
- Trait-level analysis (not broad-energy generalising). The AI extracts 6-10
  distinct trait clusters per card and finds direct mirror/opposite matches
  at the trait level, including lesser-used niche traits.
- v3 rule layer: pip-pip exception, hierarchy exception (SIM-higher vs OPP-lower
  → Draw), shared-theme draw, revised duplicate-card resolution (duplicate
  side wins when remaining card is SIM or NONE; remaining team wins on OPP).
- Historical match blocks now display as BULLETED LISTS (not dataframes).
- 'All games where this MF was the MF' block moved to the BOTTOM.
...
def _outcome_badge_html(outcome):
    """Inline coloured pill for the outcome label."""
    if outcome == 'Home':
        return ('<span style="background:#2e8b57;color:#fff;padding:1px 8px;'
                'border-radius:10px;font-size:11px;font-weight:700;">HOME</span>')
    if outcome == 'Away':
        return ('<span style="background:#1e6fb8;color:#fff;padding:1px 8px;'
                'border-radius:10px;font-size:11px;font-weight:700;">AWAY</span>')
    if outcome == 'Draw':
        return ('<span style="background:#c47a1c;color:#fff;padding:1px 8px;'
                'border-radius:10px;font-size:11px;font-weight:700;">DRAW</span>')
    return f'<span style="color:#888;">{outcome}</span>'


def _match_bullet_html(m):
    """Format one match as a single bulleted-list <li> entry."""
    outcome = m.get('outcome', '')
    badge   = _outcome_badge_html(outcome)
    score   = m.get('score', '')
    league  = (m.get('league') or '')[:32]
    h_team  = (m.get('home_team') or '')[:28]
    a_team  = (m.get('away_team') or '')[:28]
    row_idx = m.get('row_index', '')
    h, a, mf = m.get('home',''), m.get('away',''), m.get('mf','')

    teams_part = ''
    if h_team or a_team:
        teams_part = f' <span style="color:#444;">{h_team} vs {a_team}</span>'
    score_part = f' <span style="color:#666;font-size:12px;">({score})</span>' if score else ''
    league_part = f' <span style="color:#888;font-size:11px;font-style:italic;">[{league}]</span>' if league else ''
    cards_part = (f' <span style="color:#555;font-size:12px;font-family:monospace;">'
                  f'H:{h} · A:{a} · MF:{mf}</span>')
    row_part = f' <span style="color:#aaa;font-size:11px;">(row {row_idx})</span>' if row_idx else ''

    return (f'<li style="margin:4px 0;line-height:1.55;">'
            f'{badge}{teams_part}{score_part}{league_part}{cards_part}{row_part}'
            f'</li>')
...
def render_history_block(role, label, matches):
    ...
    if not matches:
        ...
        return
    # Sort by row index for stable ordering
    matches_sorted = sorted(matches, key=lambda m: m.get('row_index', 0))
    items = ''.join(_match_bullet_html(m) for m in matches_sorted)
    html = (f'<ul style="margin:6px 0 22px 18px;padding:0;list-style:disc;">'
            f'{items}</ul>')
    st.markdown(html, unsafe_allow_html=True)
...
def render_full_analysis(result, home, away, mf):
    """
    Renders the v3 trait-level analysis: per-card distinct trait clusters,
    explicit SIM/OPP connections (which MF trait connects to which team trait),
    shared-theme flag, and final rule-derived call.
    """
...
st.title("🔮 Tarot Predictor V11.1")
st.caption(
    "Winner is the team whose card best CONTROLS the Match Force — "
    "either by mirroring it (SIM) or by destabilising it (OPP)."
)
...
if st.button("Predict", type="primary", use_container_width=True):
    ...
    # ORDER (v11.1): pair blocks first, MF-alone at the bottom.
    # All blocks are bulleted lists (rather than dataframes).

    # 1. MF + Home appeared together in any position, any orientation
    #    (includes games where Home was IN the MF position)
    ...
    # 2. MF + Away appeared together in any position, any orientation
    #    (includes games where Away was IN the MF position)
    render_history_block(
        'away',
    ...
    # 3. Home + Away appeared together (any position, any orientation)
    render_history_block(
        'pair',
    ...
    # 4. All three cards together (any position, any orientation)
    render_history_block(
        'pair',
    ...
    # 5. (BOTTOM) All matches where this MF was the MF — upright AND reversed
    render_history_block(
        'mf',
        f"All matches where <code>{base_card(mf)}</code> was the MF (upright and reversed)",
...
- Remaining card is NONE → duplicated card wins → duplicate team wins
...
- Historical match bullets — MF+Home, MF+Away, Home+Away, all three, then all MF games at the bottom
""")
