...
def load_mf_lookup():
    ...
    try:
        ...
        # Columns are: 'Unnamed: 0', 'Match Force', 'Winners', 'Unnamed: 3'
        # Normalise to: card, winners, description
        cols = list(df.columns)
        # Find by position: col 1 = card, col 2 = winners, col 3 = description
        if len(cols) >= 4:
...
def matches_with_two_cards_any_position(card_a, card_b):
    """Every match where BOTH cards (any orientation) appear in any position."""
    base_a = base_card(card_a)
    base_b = base_card(card_b)
    pool = _history['co_occur'].get(base_a, [])
    return [m for m in pool if _card_appears_anywhere(m, card_b)]


def matches_with_three_cards_any_position(card_a, card_b, card_c):
    """Every match where ALL THREE cards (any orientation) appear in any position."""
    base_a = base_card(card_a)
    pool = _history['co_occur'].get(base_a, [])
    return [m for m in pool
            if _card_appears_anywhere(m, card_b)
            and _card_appears_anywhere(m, card_c)]
