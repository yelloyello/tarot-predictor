...
def _meaning_text_to_html(text):
    """
    Render a meaning string as proper HTML.
    If the text contains lines starting with '- ' (the bullet format used
    in the updated meanings sheet), render as a <ul> list. Otherwise fall
    back to plain text with line-break preservation.
    """
    if not text:
    ...
    # Treat as bulleted list if at least 2 lines and majority start with '- '
    ...
    # Single-line / no-bullet meanings — preserve line breaks
    return text.replace('\n', '<br>')
...
def _outcome_cell_html(outcome):
    """Plain text outcome label, coloured by side. No background fill / no pill."""
    colour_map = {
        'Home': '#2e8b57',   # green
        'Away': '#1e6fb8',   # blue
        'Draw': '#c47a1c',   # orange
...
def _matches_to_html_table(matches):
    """
    Build a plain, clear HTML table.
    Columns: Outcome | Home | Away | MF | Row
    Plus League / Home Team / Away Team / Score IF those fields are populated
    in the underlying data (auto-hidden when blank everywhere).
    """
...
if st.button("Predict", type="primary", use_container_width=True):
    ...
    # Plain HTML tables. Simplified headings — just the card names.

    # 1. MF + Home (includes games where Home was IN the MF position)
    ...
    # 2. MF + Away (includes games where Away was IN the MF position)
    render_history_block(
        'away',
